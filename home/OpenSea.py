...
class OpenSea:
    ...
    def GetBundles(self, limit:str = 20, offset:str=0):

        ''' Gets All symbols which are tradable (currently) '''
        params = '?&offset='+str(offset)+'&limit='+str(limit)
        url = self.base + self.endpoints['bundles'] + params
    ...
    def SingleAssetInfo(self, asset_contract_address, token_id):
        params = str(asset_contract_address) + '/' + str(token_id)
        url = self.base + self.endpoints['singleAsset'] + params
        print("========URL========",url)
        # download data
        data = requests.get(url)
        dictionary = json.loads(data.text)
